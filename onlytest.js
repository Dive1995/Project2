...
function renderList(doc){
    ...
    reserve.setAttribute('data-id',doc.id);
    ...
    reserve.addEventListener('click',(e) =>{
        e.stopPropagation();
        upid = e.target.getAttribute('data-id');
        console.log(upid);
        auth.onAuthStateChanged(user => {
            if(user){
                reserveBook(user)
                ...
                if(doc.data().noofbooks > 0){
                    ...
                    if(type == 'member'){
                        ...
                        db.collection('Reserve').add({
                            ...
                            db.collection('Books').doc(upid).update({
                                noofbooks : parseInt(doc.data().noofbooks) - 1
                            }).then(() => {
                        ...
                        alert('You are not a member yet !, please contact us.');
                    }
                    ///check availability
                }else{
                    alert('Sorry, Currently no copies available of this books !!')
                    console.log('nothing to borrow');
                }
